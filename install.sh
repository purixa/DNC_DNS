...
# ============================================================
# Headless JSON API — driven by the PDNC web panel (see /panel).
#
# Every subcommand prints exactly ONE line of JSON on success as
# the LAST line of stdout, and exits 0. On failure it may print
# human-readable diagnostics (colors included) and exits non-zero
# WITHOUT a trailing JSON line — the caller (the Go panel) treats
# any non-zero exit as an error and surfaces the captured output.
#
# These never read from /dev/tty and are safe to invoke from a
# non-interactive process (systemd service, web backend, etc).
# ============================================================
...
api_status() {
    init_state
    SERVER_IP="$(load_server_ip)"
    local u a h domains_json first=1 d

    systemctl is-active --quiet unbound && u=true || u=false
    systemctl is-active --quiet "$ADGUARD_SERVICE" && a=true || a=false
    systemctl is-active --quiet haproxy && h=true || h=false
    ...
    printf '{"ok":true,"installed":%s,"server_ip":%s,"unbound":%s,"adguard":%s,"haproxy":%s,"domains":%s}\n' \
        "$([[ -n "${SERVER_IP:-}" ]] && echo true || echo false)" \
        "$(json_str "${SERVER_IP:-}")" "$u" "$a" "$h" "$domains_json"
}
...
api_list_domains() {
    ...
    out+="]"
    printf '{"ok":true,"domains":%s}\n' "$out"
}

api_list_gamepacks() {
    local out="[" first=1 key
    for key in "${GAME_PACK_ORDER[@]}"; do
        [[ $first -eq 0 ]] && out+=","
        local dom_json="[" dfirst=1 d
        for d in $(game_pack_domains "$key"); do
            [[ $dfirst -eq 0 ]] && dom_json+=","
            dom_json+="$(json_str "$d")"
            dfirst=0
        done
        dom_json+="]"
        out+="{\"key\":$(json_str "$key"),\"label\":$(json_str "$(game_pack_label "$key")"),\"domains\":$dom_json}"
        first=0
    done
    out+="]"
    printf '{"ok":true,"gamepacks":%s}\n' "$out"
}
...
api_add_domain() {
    ...
    add_domain_core "$norm" || exit 1
    printf '{"ok":true,"domain":%s}\n' "$(json_str "$norm")"
}
...
api_remove_domain() {
    ...
    local norm
    norm="$(normalize_domain "${1:-}")"
    remove_domain_core "$norm" || exit 1
    printf '{"ok":true,"domain":%s}\n' "$(json_str "$norm")"
}
...
api_add_gamepack() {
    init_state
    local key="${1:-}"
    if [[ -z "$(game_pack_domains "$key")" ]]; then
        error "Unknown game pack: $key"
        exit 1
    fi
    add_game_pack_core "$key" || exit 1
    printf '{"ok":true,"key":%s}\n' "$(json_str "$key")"
}

api_restart() {
    init_state
    systemctl restart unbound || { error "unbound restart failed"; exit 1; }
    if adguard_service_exists; then
        systemctl restart "$ADGUARD_SERVICE" || { error "AdGuard Home restart failed"; exit 1; }
    fi
    systemctl restart haproxy || { error "haproxy restart failed"; exit 1; }
    printf '{"ok":true}\n'
}
...
dispatch_api() {
    ...
    case "${1:-}" in
        status)       api_status ;;
        list-domains) api_list_domains ;;
        add-domain)   api_add_domain "${2:-}" ;;
        remove-domain) api_remove_domain "${2:-}" ;;
        list-gamepacks) api_list_gamepacks ;;
        add-gamepack) api_add_gamepack "${2:-}" ;;
        restart)      api_restart ;;
        *)
            echo '{"ok":false,"error":"unknown api command"}'
            exit 1
            ;;
    esac
}
